...
export default function HomePage() {
  ...
  const handleSubmitUrl = async (e: React.FormEvent) => {
    ...
    if (failCount === 0) {
      ...
      setSubmitMessage(`成功提交 ${successCount} 个视频链接！`);
      setVideoUrl('');
    } else if (successCount === 0) {
      setMessageType('error');
      setSubmitMessage(`提交失败：${errors.join('; ')}`);
    } else {
      setMessageType('success');
      setSubmitMessage(`成功 ${successCount} 个，失败 ${failCount} 个`);
      setVideoUrl('');
    }
          ...
          <form onSubmit={handleSubmitUrl} className="space-y-6">
            <div>
              <label htmlFor="video-url" className="block text-sm font-medium text-gray-700 mb-2">
                视频链接 <span className="text-gray-400 font-normal">（支持批量输入，每行一个 URL）</span>
              </label>
              <textarea
                id="video-url"
                value={videoUrl}
                ...
                rows={4}
                className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-colors resize-y min-h-[100px]"
