...
我正在开发一个工具类网站，为了给用户提供更多的灵活性并满足不同的使用需求，我决定将**用户分为三类**：
...
Redis 我仍然沿用 Upstash 的免费 Redis 资源，关于 Upstash 的介绍，可以看我的这篇文章：👉《**[用 Upstash 作为你的 Redis 服务器](https://juejin.cn/post/7273025562155040780)**》。
...
```ts
ts 体验AI代码助手 代码解读复制代码// @/types/user.ts
...
```ts
ts 体验AI代码助手 代码解读复制代码// @/lib/membership/constants.ts
...
```ts
ts 体验AI代码助手 代码解读复制代码// @/app/api/membership/fake/upgrade/route.ts
...
```ts
ts 体验AI代码助手 代码解读复制代码// @/lib/membership/upgrade.ts
...
```ts
ts 体验AI代码助手 代码解读复制代码// @/app/api/membership/fake/bugBoostPack/route.ts
...
```ts
ts 体验AI代码助手 代码解读复制代码// @/lib/membership/boostPack.ts
...
```ts
ts 体验AI代码助手 代码解读复制代码// @/app/api/membership/fake/checkStatus/route.ts
...
```ts
ts 体验AI代码助手 代码解读复制代码// @/lib/membership/checkStatus.ts
...
```ts
ts 体验AI代码助手 代码解读复制代码// @/app/api/fake/useFunction/route.ts
import { NextRequest, NextResponse } from "next/server";
...
```ts
ts 体验AI代码助手 代码解读复制代码export const incrAfterUse = async ({ sub, remainingInfo }: IncrAfterChat) => {
  // 如果【默认使用次数 - 日使用次数】> 0，则自增一个日使用次数；
...
![1.png](https://p1-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/99e91c2f47c849e79cee70f88f29d1a8~tplv-k3u1fbpfcp-jj-mark:3024:0:0:0:q75.awebp#?w=1220&h=806&s=36067&e=png&b=ffffff)
...
作者：程普
链接：https://juejin.cn/post/7286133122211184700